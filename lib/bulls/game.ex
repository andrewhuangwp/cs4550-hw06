defmodule Bulls.Game do
  ...
  def guess(state, guess) do
    if validInput?(String.graphemes(guess), []) do
      ...
      bulls: state.bulls ++ [findBulls(String.graphemes(state.secret), String.graphemes(guess), 0)],
      cows: state.cows ++ [findCows(String.graphemes(state.secret), String.graphemes(guess), 0, 0)]}
    else
      state
    end
  end
